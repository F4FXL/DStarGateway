...
CTextData::CTextData(LINK_STATUS status, const std::string& reflector, const std::string& text, const in_addr& address, unsigned int port, bool temporary) :
m_status(status),
m_reflector(NULL),
m_text(NULL),
m_address(address),
...
m_temporary(temporary)
{
	...
	if (status != LS_NONE) {
		for (unsigned int i = 0U; i < reflector.size() && i < 8U; i++)
			m_reflector[i] = reflector.at(i);
	}

	for (unsigned int i = 0U; i < text.size() && i < 20U; i++)
		m_text[i] = text.at(i);
}

CTextData::CTextData(const std::string& text, const in_addr& address, unsigned int port, bool temporary) :
m_status(LS_NONE),
m_reflector(NULL),
m_text(NULL),
m_address(address),
