...
bool CDummyRepeaterProtocolHandler::writeHeader(CHeaderData& header)
{
	...
	wxLogMessage("Sending Header to port: %u, id: %04X", header.getYourPort(), header.getId());
...
bool CDummyRepeaterProtocolHandler::writeAMBE(CAMBEData& data)
{
	...
	wxLogMessage("Sending AMBE to port: %u, seq: %02X, id: %04X", data.getYourPort(), data.getSeq(), data.getId());
...
bool CDummyRepeaterProtocolHandler::writeDD(CDDData& data)
{
	...
	CUtils::dump("DD Data", buffer, length);
